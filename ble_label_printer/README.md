...
- **填充方式**：拉伸 / 适应 / 填充 / 居中 / 平铺，适配不同纸张比例。
- **无裁切水平偏移**：水平偏移通过左侧留白实现（利用打印机最大打印宽度，如 60mm），内容不会裁切。
- **参数持久化**：打印页「保存参数」按钮可将当前校准参数保存到插件设置，下次打印自动生效。
...
实际使用中在 InvenTree 后台 **标签模板** 里按需自定义；纸张尺寸由模板宽高决定，打印页上可二次微调。
...
| Fill mode | stretch | 标签图映射到纸张的方式（stretch/fit/fill/center/tile） |
